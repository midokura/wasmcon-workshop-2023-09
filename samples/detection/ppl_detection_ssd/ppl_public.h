 ...
 */
EXPORT_API EPPL_RESULT_CODE PPL_Initialize(uint32_t network_id,
                                           const char *p_param);
/**
 ...
 * @param pp_out_buf Pointer to memory address where function shall store
 * address of flat buffer
 * @param p_out_size Pointer to uint32_t where function shall store buffer size
 * of serialized flat buffer memory
 * @param p_upload_flag Pointer to flag to specify whether to upload PPL Output
 * to cloud or not.
 * @return Success or failure EPPL_RESULT_CODE
 */
EXPORT_API EPPL_RESULT_CODE PPL_Analyze(float *p_data, uint32_t in_size,
                                        void **pp_out_buf,
                                        uint32_t *p_out_size,
                                        bool *p_upload_flag);
/**
